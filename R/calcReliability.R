...
calcReliability <- function(df = NULL, model = NULL, entity = "entity", y = "y", ctrPerf = controlPerf(), ctrRel = controlRel()){
  ...
  est.HLGM.logistic <- HLGM.out$est.HLGM.logistic
  ...
  ##### compile output ####
  results <- data.frame(
    method = c('SSR', 'PSSR', 'SSR.OE', 'SSR.PE', 'PSSR.OE', 'PSSR.PE', 'ANOVA', 'Logit on logit scale', 'Logit w/ delta approx', 'Logit w/ FE', 'Logit w/ RE', 'BetaBinomial', 'BetaBinomial w/ FE', 'BetaBinomial w/ RE', 'BetaBinomial w/ Jeffreys', 'Resampling IUR'),
    ...
    within_var = c(NA, NA, NA, NA, NA, NA,
                   ...
                   median(HLGM.out$var.w.logistic),
    ...
    reliability_25p = c(
      ...
      quantile(est.HLGM.logistic, 0.25),
    ...
    reliability_75p = c(
      ...
      quantile(est.HLGM.logistic, 0.75),
