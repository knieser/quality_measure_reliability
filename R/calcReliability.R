...
calcReliability <- function(df = NULL, model = NULL, y = "y", provider = "provider", ctrPerf = controlPerf(), ctrRel = controlRel()){

  #### calculate measure performance ####
  message('calculating measure performance...')
  res <- calcPerformance(df, model, y, provider, ctrPerf)
  model = res$model
  message('...done')
  ...
  SSR.out <- calcSSR(df, model, y, provider, ctrPerf, ctrRel)
  ...
  AOV.out <- calcAOV(df, model, y, provider, ctrPerf)
  ...
  HLGM.out <- calcHLGMRel(df, model, y, provider, ctrPerf)
  ...
  betabin.out <- calcBetaBin(df, model, y, provider, ctrPerf)
  ...
  RIUR.results <- calcResamplingIUR(df, model, y, provider, ctrPerf, ctrRel)
